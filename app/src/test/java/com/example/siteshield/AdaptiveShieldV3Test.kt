...
class AdaptiveShieldV3Test {
    ...
    @Test
    fun `v5 records round trip and corrupt evidence fails safely`() {
        ...
        assertTrue(AdaptiveStateCodec.decode("v5\nbroken\tevidence").isEmpty())
    }
    ...
    fun `v3 migration preserves learned state without inventing protocol evidence`() {
        ...
        val v3Fields = encodedV4[1].split('\t').take(29).joinToString("\t")
    ...
    @Test
    fun `v4 migration does not invent host or cluster evidence`() {
        val record = learnedLoaderEngine().snapshot(10).single()
        val encodedV5 = AdaptiveStateCodec.encode(listOf(record)).lineSequence().toList()
        val v4Fields = encodedV5[1].split('\t').take(38).joinToString("\t")
        val migrated = AdaptiveStateCodec.decode("v4\n$v4Fields").single()
        assertEquals(record.protocolEvidence, migrated.protocolEvidence)
        assertEquals(0, migrated.hostAdEvidence.total)
        assertEquals(0, migrated.clusterEpisodeCount)
    }
